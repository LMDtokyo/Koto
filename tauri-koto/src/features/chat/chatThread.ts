...
function initThreadHeaderActions(): void {
  ...
  document.getElementById("thread-header-peer")?.addEventListener("click", () => {
    const id = activePeerId || activeConvId;
    if (id) mainNav.push(Screen.contact(id));
  });
...
export function initChatThread(): void {
  ...
  if (document.body.dataset.kotoThreadWs !== "1") {
    ...
    window.addEventListener("koto:ws:reaction", (ev) => {
      const frame = (ev as CustomEvent<{ conversation_id: string }>).detail;
      if (!frame || frame.conversation_id !== activeConvId) return;
      // Простое обновление: для MVP не рендерим список реакций, просто игнорируем —
      // когда появится UI реакций, перезагрузка будет осмысленной.
    });
  }
...
export async function activateChat(convId: string): Promise<void> {
  activeConvId = convId;
  activeTitle = pendingOpen.title || convId;
  ...
  if (activePeerId && /^[0-9a-f]{64}$/i.test(activePeerId)) {
    try {
      const allowed = await canMessagePeer(activePeerId);
      if (!allowed) {
        setComposerEnabled(false);
        if (status) status.textContent = "Написать можно после принятия заявки в друзья.";
      }
    } catch {
      // keep composer state if relation probe failed
    }
  }
...
function tryParseMediaEnvelope(plain: string): null | {
  type: string;
  fileId: string;
  mime: string;
  name?: string;
  ...
  try {
    const o = JSON.parse(plain) as { v?: number; type?: string; fileId?: string; mime?: string; name?: string; size?: number };
...
function openReactionPicker(anchor: HTMLElement, msgId: string): void {
  ...
  picker.style.cssText =
    "position:absolute;display:flex;gap:4px;padding:6px;border-radius:999px;background:var(--color-elevated);box-shadow:0 8px 24px rgba(0,0,0,0.45);z-index:80;";
  for (const emoji of QUICK_REACTIONS) {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.textContent = emoji;
    btn.style.cssText =
      "border:none;background:transparent;font-size:18px;cursor:pointer;padding:4px 6px;border-radius:6px;";
  ...
  picker.style.top = `${Math.max(8, r.top - 44)}px`;
