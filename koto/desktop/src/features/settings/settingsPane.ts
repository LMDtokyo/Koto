...
function syncSettingsSidebarNav(): void {
  // Обязательные элементы — без них переключение режима не имеет смысла.
  // chatlist-userbar и chatlist-settings-back-home — опциональные (они есть
  // только если в текущей вёрстке используется inline settings nav); раньше
  // их отсутствие прерывало всю функцию, и friends-mode не активировался.
  ...
  if (!top || !scroll || !sidebarMain) return;
