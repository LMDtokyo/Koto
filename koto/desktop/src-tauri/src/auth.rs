...
async fn provision_account(
    ...
    // koto-crypto использует свой собственный tokio Runtime (RT.block_on)
    // для libsignal-async вызовов. В Tauri это уже async-контекст —
    // прямой вызов = "Cannot start a runtime from within a runtime".
    // spawn_blocking уводит работу на отдельный поток без tokio-runtime,
    // где вложенный block_on безопасен.
    let bundle = tokio::task::spawn_blocking(move || {
        generate_registration_bundle_from_seed(seed_phrase, reg_id)
    })
    .await
    .map_err(|e| format!("crypto join error: {e}"))?
